...
A = sparse([eye(length(map.hat_x'), length(map.hat_x')); ones(n,1), zeros(n,length(map.hat_x')-1)]);
B = sparse([zeros(length(map.hat_x'), n); diag(ones(n, 1))]);
